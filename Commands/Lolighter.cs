...
internal static class Utils
{
   ...
   {
      if (value > 3) return value - 4; //Turn to blue
...
public class LolighterCommand : IBeatmapEditorCommandWithHistory
{
   ...
   public void Execute()
   {
      ...
      foreach (var note in notes)
      {
         ...
         // Boost Event
         if (now >= Options.ColorOffset + Options.ColorBoostSwap + boostIncrement
             && now > firstNote
             && Options.UseBoostColor)
         {
            ...
            for (var i = 0; i < calc; i++)
            {
               ...
               {
                  eventTempo.Add(
                     BasicEventEditorData.CreateNew(
      ...
      for (var i = 1; i < selections.Count; i++)
         ...
         if (notes[i].beat - notes[i - 1].beat <= 0.125
             && notes[i].beat - notes[i - 1].beat > 0
             && (notes[i].cutDirection == notes[i - 1].cutDirection
                || notes[i].cutDirection == NoteCutDirection.Any
      ...
      foreach (var note in selections) //Process specific light using time.
      {
         ...
         if ((now == time[1] || (now - time[1] <= 0.02 && time[1] != time[2]))
             && time[1] != 0.0D
             && now != last
      ...
      foreach (var note in selections) //Process all note using time.
      {
         ...
         if (time[0] >= nextSlider)
         {
            ...
            for (var i = selections.FindIndex(n => n == note); i < selections.Count - 1; i++)
               if (i != 0 && i < selections.Count)
               {
                  ...
                  else if (sliderNoteCount != 0) break;
               }
         }
         ...
         {
            if (time[1] - time[2] >= lastSpeed + 0.02
                || time[1] - time[2] <= lastSpeed - 0.02
            ...
            eventTempo.Add(
               BasicEventEditorData.CreateNew(
                  ...
                  time[0],
                  FindColor(notes.First().beat, time[0]),
                  1));
               ...
               if (selections[selections.FindIndex(n => n == note) + 1].beat == nextDouble)
               {
                  if (selections[selections.FindIndex(n => n == note) + 1].beat - time[0] <= 2)
                  {
                     var value = (selections[selections.FindIndex(n => n == note) + 1].beat
                           - selections[selections.FindIndex(n => n == note)].beat)
                        / 2;
                     eventTempo.Add(
                        BasicEventEditorData.CreateNew(
                           pattern[patternIndex],
      ...
      for (var i = 0; i < events.Count; i++)
      {
         ...
         if (mapEvent2.value == 0)
         {
            ...
            var idx = events.FindIndex(o =>
                                          o.beat == mapEvent2.beat
                                          && o.value == mapEvent2.value
                                          && o.type == mapEvent2.type);
            var r = BasicEventEditorData.CreateNew(
               events[idx].type,
               mapEvent2.beat - (mapEvent2.beat - closest) / 2,
               events[idx].value,
         ...
         {
            // Move off event between fused note and last note
            if (mapEvent.value == 0 || mapEvent.value == 4 || mapEvent.value == 8)
            {
               var idx = events.FindIndex(o =>
                                             o.beat == mapEvent.beat
                                             && o.value == mapEvent.value
                                             && o.type == mapEvent.type);
               var r = BasicEventEditorData.CreateNew(
                  events[idx].type,
                  mapEvent.beat - (mapEvent.beat - closest) / 2,
                  events[idx].value,
                  events[idx].floatValue);
               events.Remove(events[idx]);
               events.Insert(idx, r);
               ...
               events.RemoveAt(
                  events.FindIndex(o =>
                                      o.beat == mapEvent.beat && o.value == mapEvent.value && o.type == mapEvent.type));
         }
      }
